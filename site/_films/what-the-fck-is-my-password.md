---
title: "What the F*ck Is My Password?!"
description: "A self-described failure discovers that a drunken Bitcoin investment from more than a decade earlier is now worth 200 million dollars. He has four…"
year: null
type: Feature
status: in-production
director: "Steve Pink"
cast:
  - "Alexander Ludwig"
  - "Stephen Dorff"
  - "Kate Walsh"
  - "Poppy Liu"
studio: "Public House Pictures / Phiphen Pictures"
company: null
country: "USA"
runtime: null
synopsis: >
  A self-described failure discovers that a drunken Bitcoin investment from
  more than a decade earlier is now worth 200 million dollars. He has four
  password attempts and seven days to recover it before the fortune is lost.
bitcoin_angle: >
  The film turns one of Bitcoin's most recognisable real-world risks - losing
  access to self-custodied funds - into a high-concept comedy deadline. Its
  premise makes private-key responsibility part of the dramatic engine.
platform: null
trailer: null
sources:
  - label: "Deadline (Jul 2025 — Stephen Dorff, Kate Walsh cast)"
    url: "https://deadline.com/2025/07/stephen-dorff-kate-walsh-steve-pink-what-the-fuck-is-my-password-1236473729/"
  - label: "IMDb"
    url: "https://www.imdb.com/title/tt36270672/"
  - label: "Letterboxd"
    url: "https://letterboxd.com/film/what-the-fck-is-my-password/"
  - label: "Owner-review/no-publish for images (in production, no public key art)"
    url: "https://www.imdb.com/title/tt36270672/"
---