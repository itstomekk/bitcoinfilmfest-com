---
title: "Finding Satoshi"
year: 2026
type: Documentary
status: released
director: "Matthew Miele"
cast:
  - "William D. Cohan"
  - "Tyler Maroney"
  - "Michael Saylor"
  - "Fred Ehrsam"
  - "Joseph Lubin"
  - "Bill Gates"
  - "Phil Zimmermann"
  - "Bram Cohen"
  - "Kara Swisher"
studio: "Tucker Tooley Entertainment"
company: null
country: "USA"
runtime: 101
synopsis: >
  A four-year investigation into the identity of Satoshi Nakamoto, led by
  journalist William D. Cohan and private investigator Tyler Maroney. Drawing
  on digital-forensics analysis, linguistic comparison and interviews with
  people close to Bitcoin's early cypherpunk circle, the film builds a case
  that Hal Finney and Len Sassaman collaborated to create Bitcoin.
bitcoin_angle: >
  The entire film is a forensic hunt for Bitcoin's creator — testing six
  named candidates (Adam Back, Nick Szabo, Hal Finney, Len Sassaman, Paul
  Le Roux, Wei Dai) against on-chain activity patterns, writing style and
  personal history before settling on its two-person theory. Both widows,
  Coinbase CEO Brian Armstrong and security researcher Jameson Lopp appear
  on camera discussing the case.
platform: "Direct VOD release at findingsatoshi.com has closed; available to rent or own through digital video services"
trailer: null
sources:
  - label: "Official site — Tucker Tooley Entertainment"
    url: "https://www.findingsatoshi.com/"
  - label: "The Block — Finney/Sassaman conclusion"
    url: "https://www.theblock.co/post/398423/finding-satoshi-documentary-makes-case-hal-finney-len-sassaman-bitcoins-co-creators"
  - label: "IMDB"
    url: "https://www.imdb.com/title/tt40548010/"
bff_screening: "BFF'26 — June 5, 2026 (Kinoteka, Warsaw): 'who is Satoshi' overview block, screening plus producer Q&A"
featured: true
---
